...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = np.mean(-np.log(softmax_scores[np.arange(N), y])) + 0.5 * reg * (
            np.sum(W2**2) + np.sum(W1**2)
        )
        ...
        grads["W2"] = (
            relu(X @ W1 + b1).T @ (softmax_scores - np.eye(softmax_scores.shape[1])[y])
        ) / N + reg * W2
        grads["b2"] = np.mean(
            softmax_scores - np.eye(softmax_scores.shape[1])[y], axis=0
        )
        grads["W1"] = (
            X.T
            @ (
                (softmax_scores - np.eye(softmax_scores.shape[1])[y])
                @ W2.T
                * (X @ W1 + b1 > 0)
            )
        ) / N + reg * W1
    ...
    def predict(self, X):
        ...
        y_pred = softmax(relu(X @ W1 + b1) @ W2 + b2).argmax(axis=1)
        return y_pred
